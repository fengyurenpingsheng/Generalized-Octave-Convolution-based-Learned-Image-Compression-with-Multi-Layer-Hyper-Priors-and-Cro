...
def train(args):
  ...
  if args.restore_path is not None:
    ...
    saver_0.restore(sess, args.restore_path)
...
def encode(args, images_padded_numpy, size, save_output=False):
  """encode an image."""

  # Load input image and add batch dimension.
  height_ori, width_ori, height, width = size
  ...
  conditional_bottleneck_h_2 = tfc.GaussianConditional(mean=mean_h_2, scale=sigma_h_2, scale_table=scale_table)
  conditional_bottleneck_l_2 = tfc.GaussianConditional(mean=mean_l_2, scale=sigma_l_2, scale_table=scale_table)
  ...
  conditional_bottleneck_h = tfc.GaussianConditional_second(mean=mean_h, scale=sigma_h, scale_table=scale_table)
  conditional_bottleneck_l = tfc.GaussianConditional_second(mean=mean_l, scale=sigma_l, scale_table=scale_table)
  ...
  with tf.Session() as sess:
    ...
    arrays = sess.run(tensors,feed_dict={images_padded:images_padded_numpy/255.0})
    ...
    eval_bpp_h_test, eval_bpp_l_test, mse_test, psnr_test, msssim_test, num_pixels_test, x_hat_ori_test = sess.run([eval_bpp_h, eval_bpp_l, mse, psnr, msssim, num_pixels, x_hat_ori],feed_dict={images_padded:images_padded_numpy/255.0})
    ...
    psnr_yuv444, psnr_yuv444_list, msssim_y = evaluate(x_hat_ori_test,images_padded_numpy[0, top_end:top_end + height_ori, left_end:left_end + width_ori, :].astype(np.uint8))
